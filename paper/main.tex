...
Ap\'ery-like sequences arise as integral solutions of recurrence and
differential equations resembling those in Ap\'ery's proof of the
irrationality of \(\zeta(2)\) and \(\zeta(3)\); see, for example,
Zagier's classification \cite{Zagier2009}.  We consider the second-order
Legendrian sequence
...
G_{mp^r}\equiv G_{mp^{r-1}}\pmod {p^{2r}}.
\end{equation}
The modulus in \eqref{eq:Sun} gains two powers of \(p\) at each dilation.
Our argument obtains this gain directly and uniformly, without separating
the first layer from the higher layers.

Gorodetsky found a two-variable constant-term representation for
\eqref{eq:Gdef}; it appears in Section~5 of the revised arXiv version
\cite{GorodetskyV2}.  After a quadratic substitution, the representing
Laurent polynomial factors into four polynomials with coefficients in
\(\{-1,0,1\}\).  We show that the resulting multinomial expansion satisfies
an order-two Gauss congruence coefficient by coefficient.

The general Dwork congruences for a Laurent polynomial whose Newton
polytope has the origin as its unique interior lattice point naturally give
congruences of order one; see Mellit and Vlasenko \cite{MellitVlasenko}.
The extra power at every level in the present problem is therefore not a
formal consequence of that theory.  Straub \cite{Straub2014} proved
order-two, and in a subfamily order-three, multivariate supercongruences for
a partition-indexed family of rational functions.  The rational function
\(1/(1-z\Lambda_G(x,y))\) arising here is not among the displayed members
of that family, and the balanced four-row coefficient model used below is
different; hence the present theorem is not a direct specialization of
Straub's result.

Gorodetsky \cite[(6.2)]{GorodetskyV2} asks whether the Laurent polynomials
listed in his Proposition~3.3 satisfy coefficient congruences of order two
or three.  The second-order Legendrian polynomial in
\cite[(5.2)]{GorodetskyV2} is not one of the polynomials specified in that
question.  Our result is instead an analogous order-two coefficient
congruence for this Legendrian polynomial.
...
form is precisely the corresponding family of multivariate congruences.
...
The proof is elementary and \(p\)-adic, relying only on multinomial
valuations and the classical Jacobsthal--Kazandzidis congruence; no
Dwork-theoretic machinery is required.
...
The exact verification programs are provided in the public repository
